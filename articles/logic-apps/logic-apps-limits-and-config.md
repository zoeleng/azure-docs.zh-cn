...
> [!IMPORTANT]
> 如果运行的持续时间超过当前运行历史记录保留限制，则会从存储中的运行历史记录中删除该运行。 若要避免丢失运行历史记录，请确保保留限制 *始终* 大于运行的可能最长持续时间。
...
如果为逻辑应用生成 Azure 资源管理器模板，则此设置将显示为工作流资源定义中的属性，如 [Microsoft. 逻辑工作流模板引用](/azure/templates/microsoft.logic/workflows)中所述：
