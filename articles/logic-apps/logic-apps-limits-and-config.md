...
## <a name="run-duration-and-retention-history-limits"></a>运行持续时间和保留历史记录限制

下面是针对单个逻辑应用运行的限制：

| 名称 | 多租户限制 | 集成服务环境限制 | 注释 |
|------|--------------------|---------------------------------------|-------|
| 运行持续时间 | 90 天 | 366 天 | 运行持续时间是使用运行的开始时间来计算的。 |
| 在存储中运行历史记录保留期 | 90 天 | 366 天 | 如果运行的持续时间超过当前运行历史记录保留限制，则会从存储中的运行历史记录中删除该运行。 无论运行是完成还是超时，都始终使用运行时间的开始时间和工作流设置中指定的当前限制来计算运行历史记录保持期， [**以天为单位运行历史记录保持期**](#change-retention)。 无论先前的限制如何，当前限制始终用于计算保留。 <p><p>若要更改默认限制和详细信息，请参阅 [更改存储中的运行历史记录保留](#change-retention)。 若要提高最大限制，[请联系逻辑应用团队](mailto://logicappsemail@microsoft.com)，就你的要求获取帮助。 |
| 最小重复间隔 | 1 秒 | 1 秒 ||
| 最大重复间隔 | 500 天 | 500 天 ||
|||||
...
### <a name="change-run-history-retention-in-storage"></a>更改存储中的运行历史记录保留期

若要更改存储中运行历史记录保留期的默认限制，请执行以下步骤。

* 对于多租户 Azure 中的逻辑应用，90 天默认限制与最大限制相同。 只能减小此值。

* 对于集成服务环境中的逻辑应用，可以降低或提高 90 天默认限制。

例如，假设你将保留限制从90天减少到30天。 将从运行历史记录中删除60天旧运行。 如果将保持期从30天增加到60天，则将在运行历史记录中保留40一天20天的运行历史记录。 


> [!IMPORTANT]
> 若要避免丢失运行历史记录，请确保保留限制 *始终* 大于运行的可能最长持续时间。否则，运行历史记录将丢失。

1. 在 [Azure 门户](https://portal.azure.com) 搜索框中，查找并选择 " **逻辑应用**"。

1. 查找并选择逻辑应用。 在逻辑应用设计器中打开逻辑应用。
...
| 德国北部 | 51.116.211.168, 51.116.208.165, 51.116.208.175, 51.116.208.192, 51.116.208.200, 51.116.208.222, 51.116.208.217, 51.116.208.51 | 51.116.211.212, 51.116.60.192 - 51.116.60.223, 51.116.59.16 - 51.116.60.31 |
| 德国中西部 | 51.116.233.35, 51.116.171.49, 51.116.233.33, 51.116.233.22, 51.116.168.104, 51.116.175.17, 51.116.233.87, 51.116.175.51 | 51.116.236.78, 51.116.158.96 - 51.116.158.127, 51.116.155.80 - 51.116.155.95 |
