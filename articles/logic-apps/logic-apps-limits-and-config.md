...
| 在存储中运行历史记录保留期 | 90 天 | 366 天 | 当运行完成或超时时，运行历史记录保持期始终是使用运行的开始时间和工作流设置在 *当前时间* 指定的限制来计算，按 [**天运行历史记录保持期**](#change-retention)。 如果更改此设置，则始终使用 *当前* 限制来计算保留，而与以前的限制无关。 当运行的持续时间超过当前限制时，将从运行历史记录中删除运行。 <p><p>例如，假设你将保留限制从90天减少到30天。 将从运行历史记录中删除60天旧运行。 如果将保持期从30天增加到60天，则将在运行历史记录中保留40一天20天的运行历史记录。 <p><p>若要更改默认限制（即90天），请参阅 [更改存储中的运行历史记录保留](#change-retention)。 |
| 最小重复间隔 | 1 秒 | 1 秒 ||
| 最大重复间隔 | 500 天 | 500 天 ||
|||||
...
若要更改存储中运行历史记录保留期的默认限制，请执行以下步骤。 若要提高最大限制，[请联系逻辑应用团队](mailto://logicappsemail@microsoft.com)，就你的要求获取帮助。
