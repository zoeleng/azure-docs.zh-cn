...
description: '在 integration service 环境中查看网络运行状况并管理逻辑应用、连接、自定义连接器和集成帐户 (适用于 Azure 逻辑应用的 ISE) '
services: logic-apps
...
本文介绍如何 [ (ISE) 为 integration service 环境 ](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md)执行管理任务，例如：
...
1. 在 " **设置** " 下，选择 " **横向扩展** "。在 " **配置** " 窗格上，从以下选项中进行选择：
   ...
   * [**自定义自动缩放**](#custom-autoscale)：通过从各种条件中进行选择并指定满足条件的阈值条件，基于性能指标进行缩放。
   ...
   ![屏幕截图，显示 "向外扩展" 页，并选中 "手动缩放"。](./media/ise-manage-integration-service-environment/select-scale-out-options.png)
