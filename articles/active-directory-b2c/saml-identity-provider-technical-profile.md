...
| XmlSignatureAlgorithm | 否 | Azure AD B2C 用于对 SAML 请求进行签名的方法。 此元数据控制 SAML 请求中 SigAlg 参数（查询字符串或 post 参数）的值。 可能的值： `Sha256` 、 `Sha384` 、 `Sha512` 或 `Sha1` (默认) 。 确保在两端配置具有相同值的签名算法。 仅使用证书支持的算法。 |
...
| SamlAssertionDecryption |否 |  (RSA 密钥集) X509 证书。 SAML 标识提供程序使用证书的公共部分来加密 SAML 响应的断言。 Azure AD B2C 使用证书的私有部分来解密断言。 |
