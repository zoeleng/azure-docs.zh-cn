...
以下步骤假设有一个要查看其有效安全规则的现有 VM。 如果没有 VM，请先部署 [Linux](../virtual-machines/linux/quick-create-portal.md?toc=%2fazure%2fvirtual-network%2ftoc.json) 或 [Windows](../virtual-machines/windows/quick-create-portal.md?toc=%2fazure%2fvirtual-network%2ftoc.json) VM 以完成本文中的任务。 本文中的示例适用于名为 *myVM* 的 VM，其中包含名为 *myVMVMNic* 的网络接口。 VM 和网络接口在名为  针对想要诊断其问题的 VM，相应地更改步骤中的值。
...
3. 如下图所示，在“设置”下选择“网络”：  
   ...
   ![屏幕截图显示了与 V M V M Nic 的网络设置对应的 Azure 门户。](./media/diagnose-network-traffic-filter-problem/view-security-rules.png)
...
4. 确保 VM 处于运行状态，然后如上图所示选择“有效安全规则”，以查看下图所示的有效安全规则： 
   ...
   ![屏幕截图显示 "有效安全规则" 窗格，其中选择了 "下载选定的和 AllowAzureLoadBalancerInbound 入站规则"。](./media/diagnose-network-traffic-filter-problem/view-effective-security-rules.png)
   ...
   ![屏幕截图显示输入的 AllowAzureLoadBalancerInbound 的地址前缀。](./media/diagnose-network-traffic-filter-problem/address-prefixes.png)
...
5. 前面的步骤显示了名为 **myVMVMNic** 的网络接口的安全规则，但前面某些图片中也显示了名为 **myVMVMNic2** 的网络接口。 本示例中的 VM 上附加了两个网络接口。 每个网络接口的有效安全规则可能不同。
   ...
   ![屏幕截图显示了与 V M V M Nic 2 的网络设置对应的 Azure 门户。](./media/diagnose-network-traffic-filter-problem/view-security-rules2.png)
