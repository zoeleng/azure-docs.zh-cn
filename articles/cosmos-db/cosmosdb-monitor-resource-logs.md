 ...
 * **DataPlaneRequests** ：选择此选项可在 Azure Cosmos DB 中将后端请求记录到所有 API，其中包括 SQL、图形、MongoDB、Cassandra 和表 API 帐户。 要注意的关键属性包括： `Requestcharge` 、 `statusCode` 、、、 `clientIPaddress` `partitionID` `resourceTokenPermissionId` 和 `resourceTokenPermissionMode` 。
