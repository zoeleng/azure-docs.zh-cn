...
> [!IMPORTANT]
> 使用 CLI，只能创建具有内部负载均衡器的 AKS 群集。 没有用于升级现有群集的 az ml 命令来使用内部负载均衡器。
...
如果你不想使用默认的出站规则，并且希望限制对虚拟网络的出站访问权限，则必须允许访问 Azure 容器注册表。 例如，请确保网络安全组 (NSG) 包含允许访问 __RegionName__ service 标记的规则，其中 "{RegionName}" 是 Azure 区域的名称。
