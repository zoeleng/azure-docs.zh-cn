...
description: 了解如何连接到 Azure Data Science Virtual Machine (DSVM) ，以将计算能力扩展到 Azure Notebooks 预览。
author: getroyer
manager: andneil
ms.author: getroyer
ms.topic: how-to
...
- 你的帐户已连接到 Azure 订阅。
- 此订阅中有一个或多个虚拟机（至少具有读取者访问权限），该订阅使用 Linux (Ubuntu) 映像的 Data Science Virtual Machine。 ) 
...
> [!Important]
> 用户名必须是小写，才能将其用于 JupyterHub。

如果不满足任何条件，你仍可以连接到 DSVM。 在下拉列表中，选择 " **直接计算** " 选项，该选项会提示你输入要在列表中显示的名称 () 、VM 的 IP 地址和端口 (通常为8000、JupyterHub 侦听的默认端口) 和 VM 凭据：
...
当在 VM 上运行项目时，文件将装载到 Jupyter 服务器的根目录中， (JupyterHub) 中显示的目录，替换默认 Azure Notebooks 文件。 使用笔记本 UI 上的 " **关闭** " 按钮关闭 VM 时，Azure Notebooks 会还原默认文件。
