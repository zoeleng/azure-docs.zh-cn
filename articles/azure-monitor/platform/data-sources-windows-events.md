---
title: 在 Azure Monitor 中收集带有 Log Analytics 代理的 Windows 事件日志数据源
...
ms.locfileid: "92440597"
---
# <a name="collect-windows-event-log-data-sources-with-log-analytics-agent"></a>用 Log Analytics 代理收集 Windows 事件日志数据源
Windows 事件日志是 Windows 虚拟机上 Log Analytics 代理的最常见 [数据源](agent-data-sources.md) 之一，因为许多应用程序都写入 windows 事件日志。  除了指定由需要监视的应用程序创建的任何自定义日志，还可以从标准日志（如系统和应用程序）中收集事件。

> [!IMPORTANT]
> 本文介绍如何使用 [Log Analytics 代理](log-analytics-agent.md) （即 Azure Monitor 使用的代理之一）收集 Windows 事件。 其他代理收集不同的数据，并以不同的方式进行配置。 请参阅 [Azure Monitor 代理概述](agents-overview.md) ，了解可用代理的列表及其可收集的数据。

![Windows 事件](media/data-sources-windows-events/overview.png)     

## <a name="configuring-windows-event-logs"></a>配置 Windows 事件日志
从 "Log Analytics" 工作区的 "高级设置" 的 " [数据" 菜单中](agent-data-sources.md#configuring-data-sources) 配置 Windows 事件日志。

Azure Monitor 仅从在设置中指定的 Windows 事件日志收集事件。  可以通过键入日志名称并单击来添加事件日志 **+** 。  对于每个日志，仅收集具有所选严重级别的事件。  检查要收集的特定日志的严重级别。  不能向筛选事件提供任何其他条件。
...
Windows 事件记录具有一种类型的 **事件** ，并且具有下表中的属性：
