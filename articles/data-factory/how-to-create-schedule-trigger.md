...
1. 在“新建触发器”页中执行以下步骤  ： 
    ...
    1. 指定将在其中创建触发器的时区。 时区设置将应用于 " **开始日期** "、" **结束日期** " 和 "计划在高级定期选项中 **执行时间** "。 更改时区设置将不会自动更改开始日期。 请确保在指定的时区中开始日期是正确的

        > [!NOTE]
        > 对于观看夏令时的时区，触发器时间将自动调整年更改两次。 若要选择退出夏令时更改，请选择不遵守夏令时的时区（例如 UTC）

    1. 指定触发器的“重复周期”。  从下拉列表中选择一个值（“每分钟”、“每小时”、“每日”、“每周”和“每月”）。 在文本框中输入乘数。 例如，如果希望触发器每隔 15 分钟运行一次，请选择“每分钟”，并在文本框中输入  
