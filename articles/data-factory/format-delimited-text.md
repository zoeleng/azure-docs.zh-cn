...
| quoteChar        | 当列包含列分隔符时，用于括住列值的单个字符。 <br>默认值为 **双引号** `"`。 <br>如果 `quoteChar` 将定义为空字符串，则表示没有引号字符和列值，并且 `escapeChar` 用于转义列分隔符和本身。 | 否       |
