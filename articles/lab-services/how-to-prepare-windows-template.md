...
>[!IMPORTANT]
>本文包含用于简化计算机模板修改过程的 PowerShell 代码段。  对于所有显示的 PowerShell 脚本，你需要在 Windows PowerShell 中以管理员权限运行它们。 在 Windows 10 中，执行此操作的一种快速方法是右键单击 "开始" 菜单，然后选择 "Windows PowerShell (管理员) "。
...
有很多 [可以对 OneDrive 进行的自定义](/onedrive/use-group-policy)。 我们介绍一些更常见的自定义。
...
需要首先检索组织 ID。  有关更多说明，请参阅 [查找 Microsoft 365 组织 ID](/onedrive/find-your-office-365-tenant-id)。  还可以使用以下 PowerShell 获取组织 ID。
...
获得组织 ID 后，请使用以下 PowerShell 将 OneDrive 设置为提示将已知文件夹移至 OneDrive。
...
可以将 OneDrive 设置为自动登录到已登录用户的 Windows 凭据。  对于学生用学校凭据登录的类，自动登录是非常有用的。
...
此设置与在未启用 "按需 OneDrive 文件" 的设备上以无提示方式登录用户的 Windows 凭据一起使用。 如果任何用户的 OneDrive 大于指定的阈值 (（以) MB 为单位），则在 OneDrive 同步客户端 ( # A0) 下载文件之前，系统将提示其选择要同步的文件夹。  在我们的示例中，"1111-2222-3333-4444" 是组织 ID，0005000将阈值设置为 5 GB。
...
## <a name="install-and-configure-microsoft-365"></a>安装和配置 Microsoft 365

### <a name="install-microsoft-365"></a>安装 Microsoft 365

如果你的模板计算机需要 Office，则建议通过 [Office 部署工具 (ODT) ](https://www.microsoft.com/download/details.aspx?id=49117)安装 office。 你将需要使用 " [Microsoft 365 应用" 管理中心](https://config.office.com/) 来创建可重用的配置文件，以选择哪些体系结构、需要从 Office 中了解哪些功能以及更新的频率。

1. 请参阅 [Microsoft 365 Apps 管理中心](https://config.office.com/) 并下载你自己的配置文件。
...
使用 Office 配置工具可以设置 Office 接收更新的频率。 但是，如果你需要修改 Office 在安装后接收更新的频率，你可以更改更新通道 URL。 可以在 [更改组织中设备的 Microsoft 365 应用更新通道中](/deployoffice/change-update-channels)找到更新通道 URL 地址。 下面的示例演示如何将 Microsoft 365 设置为使用月度更新通道。
...
将 Windows 更新到最新版本后，你可能会考虑停止 Windows 更新。  自动更新可能会干扰计划的类时间。  如果你的课程运行时间较长，请考虑询问学生手动检查更新或在计划的类时间之外的时间设置自动更新。  有关 Windows 更新的自定义选项的详细信息，请参阅 [管理其他 Windows 更新设置](/windows/deployment/update/waas-wu-settings)。
...
本文介绍了为有效类准备 Windows 模板 VM 的可选步骤。  步骤包括安装 OneDrive 和安装 Microsoft 365、安装 Windows 更新和安装 Microsoft Store 应用的更新。  还介绍了如何设置最适合你的类的计划的更新。  
