...
> [!NOTE]
> CI/CD NuGet 即将停用。 有关如何迁移到最新 npm 的信息，请参阅[持续集成和部署概述](cicd-overview.md)
...
* [在 Visual Studio 中创建流分析项目](./stream-analytics-quick-create-vs.md)。
...
Azure Pipelines 生成管道描述了由按顺序执行的生成步骤组成的工作流。 请详细了解 [Azure Pipelines 生成管道](/azure/devops/pipelines/get-started-designer?preserve-view=true&tabs=new-nav&view=vsts)。

Azure Pipelines 发布管道描述了将应用程序程序包部署到群集的工作流。 一起使用时，生成管道和发布管道将执行从开始到结束的整个工作流，即一开始只有源文件，结束时群集中会有一个运行的应用程序。 请详细了解 Azure Pipelines [发布管道](/azure/devops/pipelines/release/define-multistage-release-process?preserve-view=true&view=vsts)。
