...
“租户各有数据库”模式可以对租户数据进行有效的隔离，但会增加需要管理和维护的数据库的数目。 [弹性作业](./elastic-jobs-overview.md)有利于多个数据库的管理。 可以使用作业安全可靠地针对一组数据库运行任务（T-SQL 脚本）。 作业可以跨应用程序中的所有租户数据库部署架构和常见的引用数据更改。 此外还可以通过弹性作业来维护数据库用来创建新租户的  模板，确保其架构和引用数据始终为最新。
...
* <bpt id="p1">**</bpt>sp<ph id="ph1">\_</ph>add<ph id="ph2">\_</ph>job<ept id="p1">**</ept> creates a job named <bpt id="p2">_</bpt>Reference Data Deployment<ept id="p2">_</ept>.
