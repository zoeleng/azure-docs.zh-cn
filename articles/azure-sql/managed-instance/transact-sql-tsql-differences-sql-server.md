...
唯一受支持的外部源类型是 RDBMS、Azure SQL 数据库和其他 Azure SQL 托管实例。 有关 PolyBase 的信息，请参阅 [PolyBase](/sql/relational-databases/polybase/polybase-guide)。
...
- 在模型数据库中创建的对象不能在 `tempdb` 重新启动或故障转移后在中自动创建，因为不 `tempdb` 会从复制的模型数据库获取其初始对象列表。 
