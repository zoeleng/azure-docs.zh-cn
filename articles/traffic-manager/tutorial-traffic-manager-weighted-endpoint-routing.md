...
在本部分，我们将在每个 Azure 区域（“美国东部”和“西欧”）各创建一个 VM（  将使用这些 VM 来测试流量管理器如何将流量路由到其权重值更高的网站终结点。
...
3. 输入或选择以下信息。 对于其他设置，请接受默认值，然后选择“确定”。 
    ...
    | 目标资源类型           | 选择“公共 IP 地址”。                           |
    | 目标资源          | 选择一个公共 IP 地址，以显示同一订阅下具有公共 IP 地址的资源的列表。 在“资源”中，选择名为  这是美国东部的 IIS 服务器 VM 的公共 IP 地址。|
    |  重量      | 输入 **100** 。        |
    |        |           |
