...
在此部分，请在“美国东部”和“西欧”Azure 区域创建两个 VM： 
...
在本部分，我们将在每个 Azure 区域（“美国东部”和“西欧”）各创建一个 VM（  稍后将根据用户的查询所在的子网，使用这些 VM 来测试流量管理器如何路由用户流量。
...
3. 输入或选择以下信息，保留剩下的默认设置，然后选择“确定”  ：
    ...
    | 目标资源类型           | 公共 IP 地址                          |
    | 目标资源          | **选择公共 IP 地址** 以显示同一订阅下具有公共 IP 地址的资源列表。 在“资源”中，选择名为  这是美国东部的 IIS 服务器 VM 的公共 IP 地址。|
...
7. 接下来，使用步骤 1-5 连接到位于“西欧”的 VM  由于 VM myVMWestEurope IP 地址与终结点 myProductionWebsiteEndpoint 关联，因此 Web 浏览器启动测试网站服务器 myIISVMWestEurope  。
