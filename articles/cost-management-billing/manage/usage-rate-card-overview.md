...
* **不同产品类型的用量** - 提供产品类型的用量数据，例如即用即付、MSDN、货币承诺、货币信用额和 EA（[CSP](/partner-center) 除外）。
...
* **支持即用即付、MSDN、货币承诺和货币信用额产品（不支持 EA 和 [CSP](/partner-center)）** - 此 API 提供 Azure 产品级费率信息。  此 API 的调用方必须传入产品/服务信息，才能获取资源详细信息和费率。 由于 EA 产品按注册自定义费率，因此我们暂时无法提供 EA 费率。
