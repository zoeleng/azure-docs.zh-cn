...
| 私有 Git 存储库– SSH – Flux 创建的密钥 | ssh：//[user@] server/存储库或 [user@] 服务器：存储库. git | Flux 生成的公钥需要添加到 Git 服务提供程序中的用户帐户。 如果将部署密钥添加到存储库而不是用户帐户，请使用 `git@` 代替 `user@` 。 在[此处](#apply-configuration-from-a-private-git-repository)可以找到更多详细信息 |
