...
**_注意：所有建议的配置值都基于 D14 v2 类型辅助角色节点_* _  
...
对于 D14 v2，推荐值为 _ * 102400 MB * *。 

#### <a name="2-determining-maximum-amount-of-memory-per-yarn-container-request"></a>**2.确定每个 YARN 容器请求的最大内存量**  
配置： * *_yarn-mb_* _

此值指示资源管理器上每个容器请求的最大分配 (MB)。 高于此指定值的内存请求将不会生效。 资源管理器可以按 _yarn 的增量为容器指定内存，最小分配-mb *，不能超过 *yarn* 指定的大小。 指定的值不应超过由“yarn.nodemanager.resource.memory-mb”指定的节点上所有容器的给定总内存。    
...
默认 HDInsight 群集在四个辅助角色节点上运行四个 LLAP 守护程序，因此建议值为 _ * 4 * *。  

#### <a name="5-tez-container-and-tez-application-master-size"></a>**5.Tez 容器和 Tez 应用程序主机大小**    
配置： * *_tez_* _. mb. 大小 _ （& e）  

_tez. mb *-定义 Tez 应用程序主机大小。  
建议的值为 4096 MB。
...
配置： * *_hive.llap.daemon.num.executors_* _， _*_llap_*_ 。
...
如果需要调整执行器的数目，则建议你按 tez * _hive 指定的那样考虑每个执行器的 4 GB 内存，并确保所有执行器所需的总内存不超过 LLAP daemon 容器可用的总内存。  
此值最多可配置为该节点上可用 vCore 总数的 75%。  
对于 D14 v2，建议的值为 (.75 X 16) = 12

**_llap_*_：此值指定执行器的线程池大小。由于执行器是按指定固定的，因此它将与每个 LLAP 后台程序的执行器数相同。对于 D14 v2，推荐值为 _* 12** 。

#### <a name="9-determining-llap-daemon-cache-size"></a>**9.确定 LLAP 守护程序缓存大小**  
配置： * *_hive. llap_* _

LLAP daemon 容器内存由以下组件组成：_ 房
*  执行程序使用的堆内存 (Xmx)
...
配置： * 配置 *_单元. hive.auto.convert.join.noconditionaltask.size_* _

请确保已启用 _hive hive.auto.convert.join.noconditionaltask.size *，此参数才会生效。
