...
|REGION|Azure 位置。 有关受支持的 Azure 位置的列表，请参阅  下拉列表框。|
|创建日期|部署群集的日期。|
...
HDInsight 群集使用 Azure 存储帐户或 Azure Data Lake Storage 来存储数据。 每个 HDInsight 群集都可拥有一个默认存储帐户和多个链接的存储帐户。 若要列出存储帐户，请在  
