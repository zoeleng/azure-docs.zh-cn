...
自定义语音使用 **基本模型** 和 **自定义模型** 。 每种语言都有一个或多个 **基本模型** 。 通常，在将新的语音模型发布到常规语音服务时，它也作为新的 **基本模型** 导入到自定义语音服务。 它们每3-6 个月更新一次，较旧的模型通常在一段时间内变得不太有用，因为最新的模型通常会显著提高准确性。

与此相反， **自定义模型** 是通过将所选基础模型适应特定的客户方案来创建的。 一旦到达满足你的需求的自定义模型，你就可以继续使用该模型，或使用其他数据在一段时间内不断地重新训练该模型。 
...
更改基本模型以添加新功能和提高性能可能会导致旧模型的向后兼容性问题，还会导致对特定测试数据集上的给定模型的准确性的更改。 若要管理维护模型和终结点的工作量，请参阅模型和终结点过期时间线的以下时间行。
