...
接下来，配置 Helm 图表值。 将以下 YAML 复制并粘贴到名为 `config-values.yaml` 的文件。 有关自定义“认知服务语音本地 Helm 图表”的详细信息，请参阅  请将 `# {ENDPOINT_URI}` 和 `# {API_KEY}` 注释替换为自己的值。
...
若要安装 Helm 图表，需要执行  下面提到的 [ Helm 图表已在`microsoft/cognitive-services-speech-onpremise`此处的 Microsoft Helm Hub][ms-helm-hub-speech-chart] 上提供。
