---
title: 教程：使用 Azure Active Directory 配置 Zscaler Private Access (ZPA) 进行自动用户预配 |Microsoft Docs
description: 了解如何配置 Azure Active Directory 以便自动预配和取消预配用户帐户，以将用户帐户预配到 Zscaler Private Access (ZPA) 。
...
本教程的目的是演示要在 Zscaler Private Access (ZPA) 和 Azure Active Directory (Azure AD) 中执行的步骤，以配置 Azure AD 自动将用户和/或组预配到 Zscaler Private Access (ZPA) 。
...
* 建议将单个 Azure AD 用户分配到 Zscaler Private Access (ZPA) ，以测试自动用户预配配置。 其他用户和/或组可以稍后分配。

* 将用户分配到 Zscaler Private Access (ZPA) 时，必须在分配对话框中选择任何特定于应用程序的有效角色 (如果可用) 。 具有“默认访问权限”  角色的用户排除在预配之外。

## <a name="set-up-zscaler-private-access-zpa-for-provisioning"></a>设置 Zscaler Private Access (ZPA) 用于预配
...
7.  向下滚动以 **启用 SCIM 同步**。单击 " **生成新令牌** " 按钮。 复制 **持有者令牌**。 此值将输入到 Azure 门户的 Zscaler Private Access (ZPA) 应用程序的 "预配" 选项卡的 "机密令牌" 字段中。
...
9.  向下滚动以查看页面末尾的 **SCIM 服务提供程序终结点** 。 复制 **SCIM 服务提供程序终结点**。 此值将输入到 "租户 URL" 字段中的 "租户 URL" 字段中的 "Zscaler Private Azure 门户)  (Access" 选项卡的 "设置" 选项卡中。
...
在将 Zscaler Private Access (ZPA) 配置为使用 Azure AD 自动进行用户预配之前，需要从) 应用程序库中将 Zscaler Private Access (ZPA Azure AD 添加到托管 SaaS 应用程序列表。
...
本部分将指导你完成以下步骤：配置 Azure AD 预配服务，以便基于 Azure AD 中的用户和/或组分配在 Zscaler Private Access (ZPA) 中创建、更新和禁用用户和/或组。

> [!TIP]
> 你还可以选择启用基于 SAML 的单一登录的 Zscaler Private Access (ZPA) 按照 [Zscaler Private access (ZPA) 单一登录教程](./zscalerprivateaccess-tutorial.md)中提供的说明进行操作。 可以独立于自动用户预配配置单一登录，尽管这两个功能互相补充。

> [!NOTE]
> 若要了解有关 Zscaler 专用访问的 SCIM 终结点的详细信息，请参阅 [此](https://www.zscaler.com/partners/microsoft)。

### <a name="to-configure-automatic-user-provisioning-for-zscaler-private-access-zpa-in-azure-ad"></a>若要配置 Zscaler Private Access 的自动用户预配 (ZPA) 在 Azure AD 中：
...
此操作会对“设置”部分的“范围”中定义的所有用户和/或组启动初始同步   。 初始同步执行的时间比后续同步长，只要 Azure AD 预配服务正在运行，大约每隔 40 分钟就会进行一次同步。 你可以使用 " **同步详细信息** " 部分监视进度并跟踪指向预配活动报告的链接，该报告描述了 Azure AD 预配服务对 Zscaler Private ACCESS (ZPA) 执行的所有操作。
