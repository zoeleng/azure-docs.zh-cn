...
本教程介绍了需要在 Webroot 安全意识培训和 Azure Active Directory (Azure AD) 中执行的步骤，以配置自动用户预配。 配置后，Azure AD 会自动预配用户和组，并取消其预配，以使用 Azure AD 预配服务 [Webroot 安全意识培训](https://www.webroot.com/) 。 有关此服务的功能、工作原理以及常见问题的重要详细信息，请参阅[使用 Azure Active Directory 自动将用户预配到 SaaS 应用程序和取消预配](../app-provisioning/user-provisioning.md)。 
...
3. 确定 [Azure AD 与 Webroot 安全意识培训之间要映射](../app-provisioning/customize-application-attributes.md)的数据。 
...
从 Azure AD 应用程序库添加 Webroot 安全意识培训，开始管理预配到 Webroot 安全意识培训。 可在[此处](../manage-apps/add-application-portal.md)详细了解如何从库中添加应用程序。 
...
* 将用户和组分配到 Webroot 安全意识培训时，必须选择 " **默认" 访问权限**以外的角色。 具有“默认访问”角色的用户将从预配中排除，并在预配日志中被标记为未有效授权。 如果应用程序上唯一可用的角色是默认访问角色，则可以[更新应用程序清单](../develop/howto-add-app-roles-in-azure-ad-apps.md)以添加其他角色。 
...
9. 在 " **属性映射** " 部分中，查看从 Azure AD 同步到 Webroot 安全意识培训的用户属性。 选为 " **匹配** " 属性的属性用于匹配 Webroot 安全意识培训中的用户帐户以执行更新操作。 如果选择更改 [匹配的目标属性](../app-provisioning/customize-application-attributes.md)，将需要确保 Webroot 安全意识培训 API 支持基于该属性筛选用户。 选择“保存”按钮以提交任何更改。
