...
在将 Elium 配置为使用 Azure AD 进行自动用户预配之前，必须在 Elium 上启用 (SCIM) 的跨域标识管理的系统。 请执行以下步骤：
...
若要将 Elium 配置为使用 Azure AD 进行自动用户预配，还必须将 Azure AD 应用程序库中的 Elium 添加到托管的软件即服务 (SaaS) 应用程序的列表中。 请执行以下步骤：
