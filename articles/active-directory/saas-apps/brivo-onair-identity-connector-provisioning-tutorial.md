---
title: 教程：配置 Brivo Onair Identity Connector 以便 Azure Active Directory 的自动用户预配 |Microsoft Docs
description: 了解如何配置 Azure Active Directory 以自动将用户帐户预配到 Brivo Onair Identity Connector 并取消其预配。
...
本教程的目的是演示要在 Brivo Onair Identity 连接器和 Azure Active Directory (Azure AD) 中执行的步骤，以将 Azure AD 自动预配和取消预配到 Brivo Onair 标识连接器。
...
* Azure AD 租户
* [Brivo Onair 标识连接器租户](https://www.brivo.com/lp/quote)
* Brivo Onair 标识连接器中具有高级管理员权限的用户帐户。
...
## <a name="important-tips-for-assigning-users-to-brivo-onair-identity-connector"></a>将用户分配到 Brivo Onair 标识连接器的重要提示

* 建议将单个 Azure AD 用户分配到 Brivo Onair Identity Connector 来测试自动用户预配配置。 其他用户和/或组可以稍后分配。

* 将用户分配到 Brivo Onair Identity Connector 时，必须在分配对话框中选择任何特定于应用程序的有效角色 (如有) 。 具有“默认访问权限”  角色的用户排除在预配之外。
...
在配置 Brivo Onair Identity Connector 以便 Azure AD 自动用户预配之前，需要将 Azure AD 应用程序库中的 Brivo Onair 标识连接器添加到托管 SaaS 应用程序列表。

**若要从 Azure AD 应用程序库中添加 Brivo Onair 标识连接器，请执行以下步骤：**
...
4. 在搜索框中，输入 **Brivo Onair Identity connector** ，在结果面板中选择 " **Brivo Onair 标识连接器** "，然后单击 " **添加** " 按钮添加该应用程序。

    ![结果列表中的 Brivo Onair Identity Connector](common/search-new-app.png)

## <a name="configuring-automatic-user-provisioning-to-brivo-onair-identity-connector"></a>配置自动用户预配到 Brivo Onair Identity Connector 
...
10. 在 " **映射** " 部分下，选择 " **将 Azure Active Directory 组同步到 Brivo Onair 标识连接器** "。
...
13. 若要为 Brivo Onair Identity Connector 启用 Azure AD 预配服务，请在 " **设置** " 部分中将 " **预配状态** " 更改为 **"打开** "。

    ![预配状态已打开](common/provisioning-toggle-on.png)

14. 通过在 " **设置** " 部分的 " **范围** " 中选择所需的值，定义要预配到 Brivo Onair 标识连接器的用户和/或组。
...
此操作会对“设置”部分的“范围”中定义的所有用户和/或组启动初始同步   。 初始同步执行的时间比后续同步长，只要 Azure AD 预配服务正在运行，大约每隔 40 分钟就会进行一次同步。 你可以使用 " **同步详细信息** " 部分监视进度并跟踪指向预配活动报告的链接，该报告描述了 Azure AD 预配服务在 Brivo Onair Identity 连接器上执行的所有操作。
