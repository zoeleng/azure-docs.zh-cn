---
title: 教程：将闪烁配置为 Azure Active Directory 的自动用户预配 |Microsoft Docs
...
本教程的目的是演示在 "闪烁" 和 "Azure Active Directory (Azure AD") 中执行的步骤，以将 Azure AD 自动预配和取消预配用户以使其闪烁。
...
在配置和启用自动用户预配之前，应确定 Azure AD 中哪些用户和/或组成员需要访问闪烁。 确定后，可按照此处的说明将这些用户和/或组分配到闪烁：
...
1. 记录 [支持案例](https://support.joinblink.com) 或电子邮件 **闪烁支持** ， support@joinblink.com 请求 SCIM 令牌。 .
...
5. 在 " **管理员凭据** " 部分中，输入 " `https://api.joinblink.com/scim` **租户 URL**"。 输入先前在**机密令牌**中检索到的**SCIM Authentication 令牌**值。 单击 " **测试连接** " 以确保 Azure AD 可以连接到闪烁。 如果连接失败，请确保闪烁帐户具有管理员权限，然后重试。
...
此操作对 "**设置**" 部分的 "**范围**" 中定义的所有用户启动初始同步。 初始同步执行的时间比后续同步长，只要 Azure AD 预配服务正在运行，大约每隔 40 分钟就会进行一次同步。 你可以使用 " **同步详细信息** " 部分监视进度并跟踪指向预配活动报告的链接，该报告描述了 Azure AD 预配服务在闪烁时执行的所有操作。
