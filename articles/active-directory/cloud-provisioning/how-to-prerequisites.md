...
### <a name="cross-domain-references"></a>跨域引用
- 如果你的用户在另一个域中有成员引用，则不会将它们同步为该用户的当前域同步。 
-  (示例：正在同步的用户的管理器在域 B 中，用户在域 A 中。同时同步域 A 和域 B 时，它们将同步，但用户管理器不会接管) 
