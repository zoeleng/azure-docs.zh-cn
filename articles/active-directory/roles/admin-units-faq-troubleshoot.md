...
**答:** 有时候，管理单元的一个或多个成员的添加或删除可能需要几分钟才能反映在“管理单元”页上。 此外，你可以直接访问关联资源的属性，查看该操作是否已完成。 有关管理单元中的用户和组的详细信息，请参阅[列出用户的管理单元](admin-units-add-manage-users.md)和[列出组的管理单元](admin-units-add-manage-groups.md)。
