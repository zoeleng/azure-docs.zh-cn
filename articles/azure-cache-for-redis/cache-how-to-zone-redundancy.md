...
适用于 Redis 标准和高级层的 Azure 缓存通过在两个专用虚拟机 (Vm) 上托管每个缓存来提供内置冗余。 即使这些 Vm 位于不同的 [Azure 容错域和更新域](../virtual-machines/manage-availability.md) 中，并且具有高可用性，它们也容易受到数据中心级故障的影响。 适用于 Redis 的 Azure 缓存还支持其高级层中的区域冗余。 区域冗余缓存在跨多个 [可用性区域](../virtual-machines/manage-availability.md#use-availability-zones-to-protect-from-datacenter-level-failures)的 vm 上运行。 它提供更高的复原能力和可用性。

> [!IMPORTANT]
> 此预览版在提供时没有附带服务级别协议，不建议将其用于生产工作负荷。 有关详细信息，请参阅 [Microsoft Azure 预览版的补充使用条款。](https://azure.microsoft.com/support/legal/preview-supplemental-terms/) 
> 
