...
ms.locfileid: "93342908"
---
# <a name="quickstart-transform-data-using-mapping-data-flows"></a>快速入门：使用映射数据流转换数据

在本快速入门中，你将使用 Azure Synapse Analytics 创建一个管道，该管道将数据从 Azure Data Lake Storage (ADLS) Gen2 源转换为使用映射数据流的 ADLS Gen2 接收器。 使用映射数据流转换数据时，可以使用本快速入门中的配置模式进行扩展
...
在本快速入门中，我们将使用名为 "adftest2020" 的工作区作为示例。 它会自动导航到 Synapse Studio 主页。
