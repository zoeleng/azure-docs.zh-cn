...
* 对于长时间运行的分布，请检查 *rows_processed* 列，以查看从该分布移动的行数是否远远多于其他分布。 如果是这样，此发现可能指示基础数据倾斜。 数据偏斜的一个原因是在包含多个 NULL 值的列上进行分布 (其行将全部位于同一分布) 中。 避免在这些类型的列上进行分布，或在可能时筛选查询以消除空值，从而避免查询速度缓慢。 
