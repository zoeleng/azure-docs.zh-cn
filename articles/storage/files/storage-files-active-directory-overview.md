...
description: Azure 文件支持通过 SMB (服务器消息块进行基于身份的身份验证) 通过 Azure Active Directory 域服务 (AD DS) 和 Active Directory。 然后，已加入域的 Windows 虚拟机 (VM) 可使用 Azure AD 凭据访问 Azure 文件共享。
author: roygara
ms.service: storage
ms.subservice: files
ms.topic: conceptual
...
-   **Azure Active Directory (Azure AD)**
    ...
    Azure Active Directory (Azure AD) 是 Microsoft 的基于云的多租户目录和标识管理服务。 Azure AD 将核心目录服务、应用程序访问管理和标识保护融入一个解决方案中。 Azure AD 联接的 Windows 虚拟机 (Vm) 无法使用 Azure AD 凭据访问 Azure 文件共享。 有关详细信息，请参阅[什么是 Azure Active Directory？](../../active-directory/fundamentals/active-directory-whatis.md)
...
-   **Azure 基于角色的访问控制 (Azure RBAC)**
    ...
    Azure 基于角色的访问控制 (Azure RBAC) 可用于对 Azure 进行细致的访问管理。 使用 Azure RBAC，可以通过向用户授予执行作业所需的最少权限，来管理对资源的访问权限。 有关 Azure RBAC 的详细信息，请参阅 [什么是 AZURE rbac) 的 azure 基于角色的访问控制 (？](../../role-based-access-control/overview.md)。
...
- Azure 文件共享仅支持以下域服务之一的基于身份的身份验证， [Azure Active Directory 域服务 (AZURE AD DS) ](#azure-ad-ds) 或 [本地 Active Directory 域服务 ](#ad-ds) (AD DS) 。
- 网络文件系统 (NFS) （预览版）不支持基于标识的身份验证方法。
...
对于本地 AD DS 身份验证，必须设置 AD 域控制器和域加入计算机或 Vm。 可以在 Azure Vm 或本地托管域控制器。 无论采用哪种方式，加入域的客户端都必须具有到域服务的线路，因此它们必须位于域服务 (VNET) 公司网络或虚拟网络中。
...
对于 Azure AD DS 身份验证，你应该启用 Azure AD 域服务和域加入你计划用于访问文件数据的 Vm。 已加入域的 VM 必须位于与 Azure AD DS 相同的虚拟网络中 (VNET) 。 
