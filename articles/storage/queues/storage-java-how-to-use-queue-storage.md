...
可以将此字符串存储在名为 ServiceConfiguration.cscfg 的服务配置文件中。 对于在 Microsoft Azure 角色内运行的应用，通过调用 **roleenvironment.getconfigurationsettings** 访问连接字符串。 下面是从名为 StorageConnectionString 的 Setting 元素获取连接字符串的示例：
