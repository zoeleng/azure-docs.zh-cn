...
- [适用于存储的 Azure 策略示例](../../governance/policy/samples/built-in-policies.md#storage)

- [用于网络的 Azure 策略示例](../../governance/policy/samples/built-in-policies.md#network)
...
- [如何配置 Azure 存储帐户的监视](./storage-monitor-storage-account.md#configure-monitoring-for-a-storage-account)
...
- [记录和检查数据](./storage-analytics-logging.md#how-logs-are-stored)
...
- [如何使用 Azure RBAC 在 Azure 门户中配置对 Azure Blob 和队列数据的访问权限](./storage-auth-aad-rbac-portal.md)
...
- [如何配置 Azure 存储帐户的指标警报](./storage-monitor-storage-account.md)
...
- [了解静态 Azure 存储加密](./storage-service-encryption.md)
...
- [AzCopy 是一个命令行实用工具，可用于在存储帐户中复制 blob、文件和表数据](./storage-use-azcopy-v10.md)
