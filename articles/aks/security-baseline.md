...
使用 Azure AD PowerShell 模块执行即席查询，以发现作为 AKS 管理组成员的帐户，并使用它来定期协调访问权限。 使用 Azure CLI 来执行 "获取托管 Kubernetes 群集的访问凭据" 等操作。 实施安全中心标识和访问管理建议。
...
• Role-Based 访问控制 (应在 Kubernetes Services 上使用 RBAC) 
