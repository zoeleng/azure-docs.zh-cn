...
此布局违反了容错域约束的 "最大差异" 保证定义。 FD0 有 2 个副本，而 FD1 有 0 个。 FD0 与 FD1 之间的总差为 2，这个总差已大于最大差值 1。 由于违反了约束，群集资源管理器不允许这种排列方式。
...
## <a name="node-buffer-and-overbooking-capacity"></a>节点缓冲区和 overbooking 容量

如果指定了指标的节点容量，则如果总负载超过指定的节点容量，则群集资源管理器将永远不会将副本放置或移动到节点。 如果群集接近于最大容量，并且必须放置、替换或移动具有较大负载的副本，则这有时会阻止放置新副本或替换失败的副本。

为了提供更大的灵活性，你可以指定节点缓冲区或 overbooking 容量。 为指标指定节点缓冲区或 overbooking 容量时，群集资源管理器将尝试以这样一种方式来放置或移动副本：缓冲或 overbooking 容量保持未使用状态，但允许在需要时使用缓冲区或 overbooking 容量来提高服务可用性，如：
...
节点缓冲区容量表示低于指定节点容量的容量的保留部分，overbooking 容量表示超出指定节点容量的额外容量的一部分。 在这两种情况下，群集资源管理器将尝试保持此容量可用。

例如，如果某个节点的指标 *cpu 利用率* 为100，而该指标的节点缓冲区百分比设置为20%，则总容量和非缓冲容量将分别为100和80，并且群集资源管理器在正常情况下不会将超过80单元负荷放置到该节点。

![总容量等于节点容量 (节点缓冲区 + 未缓冲) ](./media/service-fabric-cluster-resource-manager-cluster-description/node-capacity.png)

如果要保留仅用于增加上述服务可用性的操作的一部分节点容量，则应使用节点缓冲区。

另一方面，如果使用 node overbooking 百分比，并将其设置为20%，则总容量和非缓冲容量将分别为120和100。

![总容量等于 overbooking 容量加节点容量 (Overbooking + 无缓冲) ](./media/service-fabric-cluster-resource-manager-cluster-description/node-capacity-with-overbooking.png)

当你希望允许群集资源管理器将副本置于某个节点上，即使其总资源使用率超过容量，就应使用 Overbooking 容量。 这可用于为服务提供额外的可用性，从而降低性能。 如果使用了 overbooking，则用户应用程序逻辑需要能够使用比它可能需要的物理资源更少的资源。

如果指定了节点缓冲区或 overbooking 容量，则当目标节点上的总负载会超过总 (容量时，群集资源管理器将不会移动或放置副本（如果为 overbooking) ，则在节点缓冲区和节点容量 + overbooking 容量的情况下）。

还可以将 Overbooking 容量指定为无限大。 在这种情况下，群集资源管理器将尝试使节点上的总负载保持低于指定的节点容量，但允许在节点上放置更大的负载，这可能会导致严重的性能下降。
...
下面是一个示例，说明如何通过 *ClusterConfig.json* *Template.js* for Azure 托管的群集来指定节点缓冲区或 overbooking 容量：
